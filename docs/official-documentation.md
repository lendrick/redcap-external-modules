...
log($message[, $parameters]) | Log a message and optional array of key value pairs for later retrieval using the **queryLog()** method.  Parameters for the current **user** and **timestamp** will automatically be stored, even if the **$parameters** argument is omitted.
...
queryLog($whereSQL) | Retrieve log entries added via the **log()** method using a SQL-like where clause.  Any parameter (including the built in **user** and **timestamp**) can be specified as a SQL column name and used with standard SQL operators (ex: "timestamp > 123 and user in ('joe', 'tom') or some_parameter like '%abc%'").
